...
def update_stats_embedding(L,R,grad,block_size,b2,log_metrics=False):
    assert len(grad.shape)==2
    if type(L).__name__!='MaskedNode':
        assert type(R).__name__=='MaskedNode'
        L = b2*L + (1-b2)*jnp.sum(grad*grad,axis=-1)

    if type(R).__name__!='MaskedNode':
        assert type(L).__name__=='MaskedNode'
        R = b2*R + (1-b2)*jnp.sum(grad*grad,axis=0)
...
def update_stats(L,R,grad,block_size,b2,precond_type="all",log_metrics=False):
    ...
    if len(mgd_shape)<=1 or sum([ dim>5000 for dim in grad.shape]):
        if len(grad.shape)!=2:
            ...
            if grad.shape[0]>block_size and grad.shape[1]>block_size:
                return ShampooLRPair(L,R)
            else:
                stat =update_stats_embedding(L,R,grad,block_size,b2,log_metrics=log_metrics)
                return UpdateStats(stat=stat,
                       stat_max_ev=ShampooLRPair(L=(jnp.mean((stat.L)) if log_metrics else jnp.array(1.0)) if not isinstance(stat.L,optax.MaskedNode) else optax.MaskedNode(),
                                                 R=(jnp.mean((stat.R)) if log_metrics else jnp.array(1.0)) if not isinstance(stat.R,optax.MaskedNode) else optax.MaskedNode()))
    print(mgd_shape)
...
def update_lambdas(precond,lambd,prev_stat,grad,block_size,b3,count,exponent,precondition,matrix_epsilon,precond_type="all",log_metrics=False):
    ...
    if len(grad.shape)<=1 or sum([ dim>5000 for dim in grad.shape]):
        print('precond before filter in update_lambdas ', precond)
        print('lambd before filter in update_lambdas', lambd)
        precond = precond.L if (type(precond.L).__name__!='MaskedNode') else (precond.R if type(precond.R).__name__!='MaskedNode' else optax.MaskedNode())
        prev_stat = prev_stat.L if (type(prev_stat.L).__name__!='MaskedNode') else (prev_stat.R if type(prev_stat.R).__name__!='MaskedNode' else optax.MaskedNode())
        if type(precond).__name__!='MaskedNode' and len(precond.shape)==1:
            print('precond after filter in update_lambdas ', precond)
            print('lambd after filter in update_lambdas', lambd)
            # updating lambdas for embedding layer.
            assert len(prev_stat.shape)==1
            n = prev_stat.shape[0]

            coeff = jnp.mean(prev_stat*precond**2)
            # jax.debug.print("coeff embedd: {x} precond: {y} prev_stat: {z}",x=coeff,y=precond.mean(),z=prev_stat.mean())
            # coeff = jnp.array(1.0)
            grad_sq = (grad* (precond**2)[jnp.newaxis,:])@grad.T if grad.shape[1]>grad.shape[0] else (grad.T* (precond**2)[jnp.newaxis,:])@grad
            grad_sq = grad_sq/n
            grad_sq = jnp.zeros((block_size,block_size),dtype=jnp.float32).at[:grad_sq.shape[0],:grad_sq.shape[1]].set(grad_sq)
            res = jnp.trace(grad_sq)/grad_sq.shape[0]


            if type(lambd.L).__name__!='MaskedNode':
                lambd_old = lambd.L
                print("dtypes ", coeff.dtype,lambd_old.dtype,grad_sq.dtype)
                lambd_new = b3*coeff*lambd_old+(1-b3)*grad_sq[jnp.newaxis,jnp.newaxis]
                lambd_max_ev = jnp.einsum("ijkk->ij",lambd_new)/grad_sq.shape[0]
                return UpdateLambdas(lambd=LambdaRLPair(L=lambd_new, R=optax.MaskedNode()),
                            lambd_max_ev=ShampooLRPair(L=jnp.mean(lambd_max_ev),R=optax.MaskedNode()),
                            coeff=ShampooLRPair(L=coeff,R=optax.MaskedNode()),
                            res=ShampooLRPair(L=res,R=optax.MaskedNode()))
            else:
                lambd_old = lambd.R
                print("dtypes ", coeff.dtype,lambd_old.dtype,grad_sq.dtype)
                lambd_new = b3*coeff*lambd_old+(1-b3)*grad_sq[jnp.newaxis,jnp.newaxis]
                lambd_max_ev = jnp.einsum("ijkk->ij",lambd_new)/grad_sq.shape[0]
                return UpdateLambdas(lambd=LambdaRLPair(L=optax.MaskedNode(), R=lambd_new),
                            lambd_max_ev=ShampooLRPair(L=optax.MaskedNode(),R=jnp.mean(lambd_max_ev)),
                            coeff=ShampooLRPair(L=optax.MaskedNode(),R=coeff),
                            res=ShampooLRPair(L=optax.MaskedNode(),R=res))
        else:
...
def update_preconds_model(stats,preconds,paddings,errors,mu,
                    ...
                    batch_axis_name,
                    precond_type):
    print("stats in update_preconds",stats)
    print("preconds in update_preconds",preconds)
    print("paddings in update_preconds",paddings)
    #remove the stats from embedding layers
    stats = jax.tree_util.tree_map(lambda x: x if len(x.shape)!=1 else optax.MaskedNode(),stats)
    preconds = jax.tree_util.tree_map(lambda x: x if len(x.shape)!=1 else optax.MaskedNode(),preconds)

    print("stats after in update_preconds",stats)
    print("preconds after in update_preconds",preconds)
    print("paddings after in update_preconds",paddings)
    stats_flat,tree_def = jax.tree_util.tree_flatten(stats)
    paddings_flat,_ = jax.tree_util.tree_flatten(paddings)

    print("paddings_flat",paddings_flat)
    # jax.debug.print("update_preconds_model  : {L}", L=paddings_flat)
    preconds_flat,tree_def = jax.tree_util.tree_flatten(preconds)
    print("preconds_flat", preconds_flat)
    ...
    print("preconds_flat in update_fn",preconds_flat)
    print("treedef ", tree_def)
...
def caspr_update_fn(precond,momentum,adam_update,precond_lambd,block_size,caspr_p=2,global_grafting=False,exponent=1,precond_type="all"):
    ...
    if len(adam_update.shape)==1 or sum([ dim>5000 for dim in adam_update.shape]):
        ...
        precond = precond.L if (type(precond.L).__name__!='MaskedNode') else (precond.R if type(precond.R).__name__!='MaskedNode' else optax.MaskedNode())
        precond_lambd = precond_lambd.L if (type(precond_lambd.L).__name__!='MaskedNode') else (precond_lambd.R if type(precond_lambd.R).__name__!='MaskedNode' else optax.MaskedNode())
        if type(precond).__name__!='MaskedNode' and len(precond.shape)==1:
            print("precond in caspr_update_fn", precond)
            print("precond_lambd in caspr_update_fn", precond_lambd)
            assert len(momentum.shape)==2
            d1,d2 = momentum.shape
            if d1>d2:
                momentum = (precond[:,jnp.newaxis]*momentum)@precond_lambd[0,0,:d2,:d2]
            else:
                print("d1,d2",d1,d2,"precond_lambd",precond_lambd.shape)
                momentum = precond_lambd[0,0,:d1,:d1]@(momentum*precond[jnp.newaxis,:])
            momentum = momentum/(1e-30+jnp.linalg.norm(momentum.reshape(-1))) * jnp.linalg.norm(adam_update.reshape(-1))
            return momentum
        else:
            return adam_update
...
def get_paddings(s,G,block_size):
    ...
    if len(mgd_shape)<=1 or sum([ dim>5000 for dim in G.shape]):
        if len(G.shape)!=2:
            #for biases
            return ShampooLRPair(R=optax.MaskedNode(),L=optax.MaskedNode())
        else:
            #for embedding
            #consider the case when embedding is not tall and thin anymore.
            if G.shape[0]>block_size and G.shape[1]>block_size:
                return ShampooLRPair(L=optax.MaskedNode(),R=optax.MaskedNode())
            if G.shape[0]>G.shape[1]:
                return ShampooLRPair(L=optax.MaskedNode(),R=jnp.asarray(np.ones((1,1))*G.shape[1]))
            else:
                return ShampooLRPair(L=jnp.asarray(np.ones((1,1))*G.shape[0]),R=optax.MaskedNode())
        return ShampooLRPair(L=optax.MaskedNode(),R=optax.MaskedNode())
    print(mgd_shape)
    blkd_shape = get_blocked_shape(mgd_shape,block_size)
    g1,g2,_,_ = blkd_shape
...
def scale_by_caspr(
        ...
        def stat_and_precond_init(param,state_type='stats'):
            ...
            if len(param.shape) > 1 and not sum([dim>5000 for dim in param.shape]):
                ...
                if len(param.shape)!=2:
                    ...
                    if param.shape[0]>block_size and param.shape[1]>block_size:
                        return ShampooLRPair(L=optax.MaskedNode(),R=optax.MaskedNode())
                    else:
                        large_axis_0 = (param.shape[0]>param.shape[1])
                        coeff = matrix_epsilon if state_type=='stats' else 1.0
                        st_L = jnp.ones(param.shape[0])*coeff if large_axis_0 else optax.MaskedNode()
                        st_R = jnp.ones(param.shape[1])*coeff if not large_axis_0 else optax.MaskedNode()
                        return ShampooLRPair(L=st_L,R=st_R)
            ...
            if len(param.shape) >1 and not sum([dim>5000 for dim in param.shape]):
                ...
                if len(param.shape)!=2:
                    ...
                    if param.shape[0]>block_size and param.shape[1]>block_size:
                        ...
                        large_axis_0 = (param.shape[0]>param.shape[1])
                        blkd_shape = (1,1,block_size,block_size)
                        d1,d2 = param.shape[0],param.shape[1]
                        st = jnp.zeros(blkd_shape)
                        st = st.at[:,:].set(jnp.eye(block_size)*(np.arange(block_size)<min(d1,d2)))
                        st_L = st if not large_axis_0 else optax.MaskedNode()
                        st_R = st if large_axis_0 else optax.MaskedNode()
                        return LambdaRLPair(R=st_R,L=st_L)
                ...
                if len(param.shape)!=2:
                    ...
                    if param.shape[0]>block_size and param.shape[1]>block_size:
                        ...
                        if param.shape[0]>param.shape[1]:
                            #use lambda for R and stat for L
                            l_metrics = TrainingMetrics(root_errors=optax.MaskedNode(),root_failure_perc=optax.MaskedNode(),
                                                        root_errors_lambdas=optax.MaskedNode(),root_failure_perc_lambdas=optax.MaskedNode(),coeff=optax.MaskedNode())
                            r_metrics = TrainingMetrics(root_errors=optax.MaskedNode(),root_failure_perc=optax.MaskedNode())
                        else:
                            r_metrics = TrainingMetrics(root_errors=optax.MaskedNode(),root_failure_perc=optax.MaskedNode(),
                                                        root_errors_lambdas=optax.MaskedNode(),root_failure_perc_lambdas=optax.MaskedNode(),coeff=optax.MaskedNode())
    ...
    def update_fn(updates, state, params=None):
        ...
        print("stats ",stats)
        print("preconds in update_fn",state.preconds)
        # get preconditioners and collect pth root metrics
        ...
        preconds_with_embedd = jax.tree_util.tree_map(lambda p,s: 1/jnp.sqrt(s+eps) if type(s).__name__!='MaskedNode' and len(s.shape)==1 else p ,
                                                      preconds, stats, is_leaf = lambda x: type(x).__name__ in ['MaskedNode'])
        ...
        lambda_updates = jax.tree_util.tree_map(lambda p,l,s,u:update_lambdas(p,l,s,u,block_size,b3,count_inc,exponent,
                                                                                ...
                                                                                log_metrics),
                                          preconds_with_embedd,state.lambdas,state.oldstats,updates,
        ...
        #get the regularized old statistics used to compute the coefficients
        ...
        def regularized_stat(stat,update):
            ...
            if type(L).__name__!='MaskedNode':
                if len(L.shape)==1:
                    #embedding
                    return ShampooLRPair(L=L+eps,R=optax.MaskedNode())
            ...
            if type(R).__name__!='MaskedNode':
                if len(R.shape)==1:
                    #embedding
                    return ShampooLRPair(L=optax.MaskedNode(),R=R+eps)
                mgd_shape = get_merged_shape(update.shape)
                blkd_shape = get_blocked_shape(mgd_shape,block_size)
                paddings = get_paddings(stat, update, block_size)
        ...
        oldstats = jax.tree_util.tree_map(lambda s,os: s if len(s.shape)==1 else os,stats,oldstats)
        print("oldstats in update_fn", oldstats)
        ...
        caspr_updates = jax.tree_util.tree_map(
          ...
          preconds_with_embedd,mu_hat,adam_updates,preconds_lambdas,
