...
def update_lambdas(precond,lambd,prev_stat,grad,block_size,b3,count,exponent,precondition):
		print('prev lambda', lambd.L.shape)
		if len(grad.shape)<=1 or sum([ dim>20000 for dim in grad.shape]):
				return lambd
		mgd_shape = get_merged_shape(grad.shape)
		grad = grad.reshape(mgd_shape)
		grad = jnp.pad(grad,((0,(-mgd_shape[0])%block_size),(0,(-mgd_shape[1])%block_size)),mode='constant')
		...
		if exponent==2:
				lambdL_coeff = jax.lax.cond(precondition,
								lambda: tr(Pr_t,mm(prev_stat.R,Pr_t))[:,:,jnp.newaxis]/N[:,:,np.newaxis],
								lambda: jnp.ones((g1,g2,1)))

		else:
				lambdL_coeff = jax.lax.cond(precondition,
								lambda: tr(Pr_t,prev_stat.R)[:,:,jnp.newaxis]/N[:,:,np.newaxis],
								lambda: jnp.ones((g1,g2,1)))

		#computing diag(G_tPr_tG_t^T)/n
		if exponent!=2:
				right_grad = jnp.einsum("ijkl,ijlm->ijkm",grad,Pr_t)
				lambdL_res = (grad*right_grad).sum(axis=-1)/N[:,:,np.newaxis]
		else:
				right_grad = jnp.einsum("ijkl,ijlm->ijkm",grad,Pr_t)
				lambdL_res = (right_grad*right_grad).sum(axis=-1)/N[:,:,np.newaxis]


		lambdL = jnp.clip(b3*lambd.L*lambdL_coeff +(1-b3)* lambdL_res,1e-30,1e30) 

		Pl_t = precond.L
		if exponent==2:
				lambdR_coeff = jax.lax.cond(precondition,
											lambda: tr(Pl_t, mm(prev_stat.L,Pl_t))[:,:,jnp.newaxis]/M[:,:,np.newaxis],
											lambda: jnp.ones((g1,g2,1)))
		else:
				lambdR_coeff = tr(Pl_t, prev_stat.L)[:,:,jnp.newaxis]/M[:,:,np.newaxis]

		#computing diag(G_t^TPl_tG_t)/m
		if exponent!=2:
				left_grad = jnp.einsum("ijkl,ijlm->ijkm",Pl_t,grad)
				lambdR_res = (grad*left_grad).sum(axis=-2)/M[:,:,np.newaxis]
		else:
				left_grad = jnp.einsum("ijkl,ijlm->ijkm",Pl_t,grad)
				lambdR_res = (left_grad*left_grad).sum(axis=-2)/M[:,:,np.newaxis]
		lambdR = jnp.clip(b3*lambd.R*lambdR_coeff + (1-b3)*lambdR_res,1e-30,1e30) 
...
def caspr_update_fn(precond,momentum,adam_update,lambd,block_size,caspr_p=2,global_grafting=False,exponent=1,lamb_eps=1e-2):
		...
		if caspr_p==2:
				...
				proc_lambdR = lamb_eps*jnp.max(lambd.R,axis=-1)[:,:,jnp.newaxis]+lambd.R
				...
				proc_lambdL = lamb_eps*jnp.max(lambd.L,axis=-1)[:,:,jnp.newaxis]+lambd.L
...
def scale_by_caspr(
		...
		def init_fn(params):
				...
				def lambda_init(param):
						...
						if len(param.shape) >1 and not sum([dim>20000 for dim in param.shape]):
								...
								print('lambdR',lambdR.shape,'lambdL',lambdL.shape)
								return LambdaRLPair(R=lambdR,L=lambdL)
						else:
								blkd_shape = (1,)
				...
				return ScaleByCasprState(count=jnp.zeros([], jnp.int32), mu=mu, nu=nu, stats=stats, preconds=preconds, lambdas=lambdas, oldstats=oldstats)
		...
		def update_fn(updates, state, params=None):
				...
				#update momentum and elem-wise second moments
				...
				caspr_updates = jax.tree_util.tree_map(
						lambda p,m,u,l: caspr_update_fn(p,m,u,l,block_size,caspr_p,global_grafting,exponent,lamb_eps),
				...
				return updates, ScaleByCasprState(count=count_inc, mu=mu, nu=nu, stats=stats, preconds=preconds, lambdas=lambdas,oldstats=oldstats)
...
def efficient_caspr_adaptive_dist_inv_v2(
